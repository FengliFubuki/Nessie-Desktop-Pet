Create one horizontal look-direction strip for Codex pet `nessie`, atlas row 10.
...
REVISED180: current approveddown anchor has the same forward/right neckcurl family as row9's final157.5. Use cardinal anchor for clearly DOWN snout meaning, but use COMPLETED row9 for actual head height, body scale and neck position. Start180 as one modest step after row9final157.5, not a higher independently normalized pose. Then turn toward down-left smoothly, gradually bringing the head through center and left. No jump at157.5→180 or337.5→000. Match completed row9 source-canvas body size and output dimensions so the shared normalization scale stays valid. Preserve torso/tail/flippers at same anchor throughout.


BOUNDARY REPAIR: the first180 previously flipped the neck left and failed. Use the enlarged157.5 exact registered pose at bottomLEFT of the boundary board as the start construction. FIRST180 MUST KEEP THE SAME RIGHTWARD NECK CURL AND HEAD LOCATION/HEIGHT, with only a small pitch turning its nose verticallydown. Do not make180 a left-facing head or a leftward neckcurl.202.5 may start yawing the nose left while retaining the same rightcurled neck and head position.225/247.5 gradually soften the neck towardcenter/left, with no instantaneous sideflip.270 must be clear left profile;292.5/315/337.5 gradually rise to bottomRIGHT000up frame. Body/tail/flippers never change sides. The whole row is still drawn as one coherent generation.
